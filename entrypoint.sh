...
mkdir -p $CLONE_DIR/$INPUT_DESTINATION_FOLDER$INPUT_TARGET_DIR_NAME
if [ "$INPUT_TARGET_DIR_NAME" ]
then 
  mv $INPUT_SOURCE_FILE/ $INPUT_TARGET_DIR_NAME/
  sh -c "rsync -rca --delete $INPUT_TARGET_DIR_NAME/ $CLONE_DIR/$INPUT_DESTINATION_FOLDER$INPUT_TARGET_DIR_NAME/"
